...
describe('GET /api/scores/best', () => {
  ...
  it('should return best score for authenticated user and valid game', async () => {
    ...
    const url = new URL('http://localhost:4321/api/scores/best?gameId=tetris')
    ...
    expect(getUserBestScoreByGame).toHaveBeenCalledWith('user-123', 'tetris')
  })
  ...
  it('should handle multiple valid game IDs', async () => {
    ...
    for (let i = 0; i < gameIds.length; i++) {
      ...
      vi.mocked(getUserBestScoreByGame).mockResolvedValue(expectedScores[i])
