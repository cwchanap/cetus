...
describe('GET /api/scores/history', () => {
  ...
  it('should return game history with custom limit', async () => {
    // Arrange
    vi.mocked(auth.api.getSession).mockResolvedValue(mockSession)
    vi.mocked(getUserGameHistory).mockResolvedValue(mockGameHistory.slice(0, 5))
  ...
  it('should return 400 for invalid limit parameter (too large)', async () => {
    ...
    const url = new URL('http://localhost:4321/api/scores/history?limit=101')
    const request = new Request(url)
  ...
  it('should return 400 for invalid limit parameter (non-numeric)', async () => {
    ...
    const url = new URL('http://localhost:4321/api/scores/history?limit=abc')
    const request = new Request(url)
  ...
  it('should return 500 for database errors', async () => {
    // Arrange
    vi.mocked(auth.api.getSession).mockResolvedValue(mockSession)
    vi.mocked(getUserGameHistory).mockRejectedValue(new Error('Database error'))
  ...
  it('should handle edge case limits correctly', async () => {
    ...
    for (const { limit, expected } of testCases) {
      ...
      // Assert
      expect(response.status).toBe(200)
      expect(getUserGameHistory).toHaveBeenCalledWith('user-123', expected)
    }
  })
})
