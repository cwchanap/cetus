...
export const POST: APIRoute = async ({ request, locals }) => {
    ...
    try {
        ...
        if (!allowedMimeTypes.includes(avatarEntry.type)) {
        ...
        const buffer = Buffer.from(await avatarEntry.arrayBuffer())
        const base64Image = `data:${avatarEntry.type};base64,${buffer.toString('base64')}`
